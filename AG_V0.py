...
def programa():
    ...
    seq_menor_dist = []
    pai12 = []
    # se a quantidade de medicamentos for menor que 10, será encontrada a distância para cada solução possível e será retornada a sequência que obteve a menor distância
    if qtd_medicamento <= 9:
        for i in range(qtd_medicamento):
            elemento.append(i+1)
        permutacoes = itertools.permutations(elemento)

        # recupera todas as possibilidades de sequência
        for permutacao in permutacoes:
            listas.append(permutacao)

        # calcula a distância para cada sequência gerada e as salva numa lista
    ...
    # se a quantidade de medicamentos maior que 9, então será aplicado o algoritmo genético para se obter uma solução ótima
    else:
